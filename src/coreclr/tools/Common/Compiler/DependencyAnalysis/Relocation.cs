...
namespace ILCompiler.DependencyAnalysis
{
    public enum RelocType
    {
        ...
        IMAGE_REL_BASED_PPC64_REL16          = 0x22,   // PPC64: addis+addi PC-relative address
        IMAGE_REL_BASED_PPC64_TOC16          = 0x23,   // PPC64: addis+addi TOC-relative address
        IMAGE_REL_BASED_PPC64_TPREL16        = 0x24,   // PPC64: addis+addi thread-pointer-relative address
        IMAGE_REL_BASED_PPC64_GOT_TPREL16    = 0x25,   // PPC64: addis+ld+add GOT thread-pointer-relative access
        IMAGE_REL_BASED_PPC64_GOT16          = 0x26,   // PPC64: addis+ld GOT function address access
        IMAGE_REL_BASED_PPC64_REL16_TOC      = 0x27,   // PPC64: addis+addi PC-relative TOC address for global entry
    ...
    public struct Relocation
    {
        ...
        private static unsafe int GetPpc64Rel24(uint* pCode)
        {
            uint branchInstr = *pCode;
            return ((int)(branchInstr & 0x03FFFFFC) << 6) >> 6;
        }

        public static bool FitsInPpc64Rel24(long offset)
        {
            return (offset >= -0x02000000L) && (offset < 0x02000000L) && ((offset & 0x3) == 0);
        }

        private static unsafe void PutPpc64Rel24(uint* pCode, long offset)
        {
            Debug.Assert(FitsInPpc64Rel24(offset));

            uint branchInstr = *pCode;

            // Keep primary opcode, AA, and LK bits; replace the LI field.
            branchInstr &= 0xFC000003;
            branchInstr |= (uint)offset & 0x03FFFFFC;
        ...
        private static unsafe short GetPpc64Imm16(uint* pCode)
        {
            return unchecked((short)(*pCode & 0xFFFF));
        }

        private static unsafe void PutPpc64Imm16(uint* pCode, long value)
        {
            uint instr = *pCode;
            ...
            *pCode = instr;
            ...
            Debug.Assert(GetPpc64Imm16(pCode) == unchecked((short)value));
        }

        private static unsafe long GetPpc64HaLo(uint* pCode)
        {
            return ((long)GetPpc64Imm16(pCode) << 16) + GetPpc64Imm16(pCode + 1);
        }

        private static unsafe void PutPpc64HaLo(uint* pCode, long value)
        {
            PutPpc64Imm16(pCode, (value + 0x8000) >> 16);
            PutPpc64Imm16(pCode + 1, value);

            Debug.Assert(GetPpc64HaLo(pCode) == value);
        }
        ...
        public static readonly int MaxSize = 12;
        ...
        public static int GetSize(RelocType relocType)
        {
            return relocType switch
            {
                ...
                RelocType.IMAGE_REL_BASED_PPC64_REL16 => 8,
