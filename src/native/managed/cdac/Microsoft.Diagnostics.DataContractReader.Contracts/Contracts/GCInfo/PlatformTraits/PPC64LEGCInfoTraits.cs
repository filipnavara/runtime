...
internal class PPC64LEGCInfoTraits : IGCInfoTraits
{
    public static uint DenormalizeStackBaseRegister(uint reg) => reg == 0 ? 31u : 1u;
    ...
    // PPC64LE volatile integer registers r3-r12 may only be reported for the active frame.
    // See gcinfodecoder.cpp IsScratchRegister for TARGET_POWERPC64.
    public static bool IsScratchRegister(uint regNum) => regNum >= 3 && regNum <= 12;
}
