...
internal class PPC64LEFrameHandler(Target target, ContextHolder<PPC64LEContext> contextHolder) : BaseFrameHandler(target, contextHolder), IPlatformFrameHandler
{
    private readonly ContextHolder<PPC64LEContext> _holder = contextHolder;
    ...
    public void HandleHijackFrame(HijackFrame frame)
    {
        ...
        Debug.Assert(hijackArgsSize % 8 == 0, "HijackArgs contains register values and should be a multiple of the pointer size (8 bytes for PPC64LE)");
